...
ggplot(df, aes(x = year)) + 
  ...
  theme(axis.text.y.right = element_text(color = "blue", angle = 270, hjust = 0.5)) +
...
p1/p2 + plot_layout(heights = c(4, 2))
...
На Fig 6 в глаза бросается относительная большая специализация России на Философии? (это ЛОГОС???))))) социологии, а так же относительно большая специализация США на литературе. У России в структуре относительно мало Истории, а у США экономики.
...
df <- data %>% distinct(UT, .keep_all = TRUE) %>%
  ...
  mutate(research_areas = case_when(n() < 400 ~ "Other (< 400 papers)",
                                    TRUE ~ research_areas)) %>% ungroup() %>% 
filter(region != "Unknown")  %>% filter(region != "Other") %>% 
