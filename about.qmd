...
p1 <- df %>% ggplot(aes(x=year, y=mean_TC)) +#, fill=region)) + 
  ...
  theme_test() +
  theme(strip.background = element_rect(fill = NA),
        strip.text = element_text(colour = 'black', size = sz)) +
  theme(panel.grid = element_blank(),
        legend.position = "none",
...
p3 <- df %>%   
  ggplot(aes(x=year, fill=zero_cit)) + 
  geom_bar(position = "fill",
           alpha = 0.7 , size = 0.1, colour = "black") +
  scale_y_continuous(labels=scales::percent) +
  geom_hline(yintercept = 0.5, linetype = "dashed", size=0.3) +
  facet_wrap(~region, nrow = 1) +
  labs(fill = "") +
...
p1/p2/p3
ggsave(filename = "II_Fig1_3_v2.tiff",
       family="Helvetica",
...
p1 <- df %>% ggplot(aes(x=year, value, color=top_x, group = top_x)) + 
  ...
  labs(color = "top:") +
  scale_color_manual(values = c("#40126e", "#1F78B4", "#A6CEE3")) +
  theme_test() +
...
#| eval=FALSE
df <- data %>% distinct(UT, region, .keep_all = TRUE) %>% 
  select(year, region, Q) %>% 
  filter(region != "Unknown") %>% 
  mutate(region = factor(region, levels = regions)) %>% 
  filter(region != "Other") %>% 
  mutate(Qv2 = case_when(Q == "Q1" ~ 1,
                        TRUE ~ 0)) %>% 
  group_by(year, region) %>% mutate(count = length(Qv2)) %>% ungroup() %>% 
  group_by(year, region, count) %>% summarise(Qv2 = sum(Qv2)) %>% ungroup() %>% 

  mutate(`Q1%` = 100 * Qv2 / count) 



df %>% ggplot(aes(x=year, `Q1%`)) + 
  geom_line(alpha = 0.7, size = 0.5, color = "#40126e") +
  geom_point(alpha = 0.7, size = 0.5, color = "#40126e") +
  # geom_ma(ma_fun = SMA, n = 3, linetype = "solid", size = 1.1) +
  # scale_x_continuous(
  #       labels = as.character(seq(min(df$year), max(df$year), by = 1)),
  #       breaks = seq(min(df$year), max(df$year), by = 1),
...
df %>% ggplot(aes(x=year, `Q1%`, color = region)) + 
  geom_line(alpha = 0.2, size = 0.5) +
  geom_point(alpha = 0.2, size = 0.5) +
  geom_ma(ma_fun = SMA, n = 3, linetype = "solid", size = 1.1) +
  # scale_x_continuous(
...
data %>% distinct(UT, region, .keep_all = TRUE) %>% 
  ...
  # facet_wrap(~field, nrow = 1) +
  labs(y = "top_10", color = "") +
  guides(color = guide_legend(keyheight = 0.6, keywidth = 0.6)) + 
...
```{r fig78}
#| eval=FALSE
p1 <- data %>% distinct(UT, region, .keep_all = TRUE) %>% 
  filter(region != "Unknown") %>% 
  # separate_rows(field, sep=";") %>% 
  filter(top_10 == 1) %>% 
  group_by(year, region) %>% count() %>% ungroup() %>% 
  group_by(year) %>% mutate(`top 10%` = 100 * n / sum(n)) %>% 
  mutate(region = factor(region, levels = regions)) %>% select(-n)
p2 <- data %>%  
  distinct(UT, region, .keep_all = TRUE) %>% 
  filter(region != "Unknown") %>% 
  filter(Q == "Q1") %>% filter(year >= 2000) %>% 
  group_by(year, region) %>% count() %>% ungroup() %>% 
  group_by(year) %>% mutate(Q1 = 100 * n / sum(n))  %>% select(-n)
p <- left_join(p1, p2, by = c("year", "region"))  

p <- p %>% pivot_longer(-c(year, region))
p <- p %>% filter(!is.na(value)) %>% mutate()

p %>%  mutate(region = factor(region, levels = regions),
              name = factor(name, levels = c("top 10%", "Q1"))) %>% 
...
ggsave(filename = "Fig78.tiff",
       family="Helvetica",
       units = "mm", dpi = 600,  width = 174,
       height = 86)

ggsave(filename = "Fig78_v1.tiff",
       family="Helvetica",
       units = "mm", dpi = 600,  width = 174,
       height = 76)
```
...
ggsave(filename = "Fig78_v2.tiff",
       family="Helvetica",
       units = "mm", dpi = 600,  width = 174,
       height = 86)

ggsave(filename = "Fig78_v3.tiff",
       family="Helvetica",
       units = "mm", dpi = 600,  width = 174,
       height = 72)
```
...
## 8. Доля региона в топ10 vs Доля региона во всех публикациях

```{r}
df1 <- data %>% distinct(UT, region, .keep_all = TRUE) %>%
  filter(region != "Unknown") %>%
  filter(top_10 == 1) %>% 
  group_by(year, region) %>% count() %>% rename(top_10 = n)

df2 <- data %>% distinct(UT, region, .keep_all = TRUE) %>% 
  filter(region != "Unknown") %>% 
  group_by(year, region) %>% count() %>% rename(tot_pub = n)


df <- df2 %>% left_join(df1, by = c("year", "region")) %>%
  filter(year >= 2000) %>% 
  mutate(top_10 = case_when(is.na(top_10) ~ as.numeric(0),
                            TRUE ~ as.numeric(top_10))) %>% 
  group_by(year) %>% mutate(region = region,
                               top_10_sh = 100 * top_10 / sum(top_10),
                               tot_pub_sh = 100 * tot_pub / sum(tot_pub)) %>% 
  ungroup() %>% 
  mutate(sh = 100 * top_10 / tot_pub)

p2 <- df %>% select(year, region, top_10_sh, tot_pub_sh) %>% 
  pivot_longer(-c(year,region)) %>% 
  ggplot(aes(x = year, value, color = name)) + 
  geom_line(size = 0.6) +
  geom_point(size = 1.1) +
  # scale_y_continuous(labels = function(x) paste0(x, "%"),
  #                    breaks = c(0, 15, 30, 45, 60)) +
  labs(title = "Fig 9: Доля региона в Топ10 vs Доля региона во всех публикациях") +
  theme_bw() +
...
ggplotly(p2) %>% layout(legend = list(orientation = "h",   
                                      xanchor = "center",  
                                      x = 0.5,
                                      y = -0.25))  
```
