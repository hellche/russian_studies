...
data %>% #distinct(UT, .keep_all = TRUE) %>% 
  ...
  select(country, n_I, n_II, n_III,tc_I,tc_II,tc_III,mncs_I,mncs_II,mncs_III) %>% 
datatable(rownames = FALSE, 
  ...
  DT::formatStyle(columns = c(1:10), fontSize = '80%') %>%  
...
p2 <- data %>% filter(region != "Unknown") %>% 
  ...
  labs(title = "Fig 7: Доли регионов в самом лучшем аутпуте",
      subtitle = "светалая линия - реальные значения; темная линия - скользящее среднее (шаг = 3 года)") +
  theme_bw() +
